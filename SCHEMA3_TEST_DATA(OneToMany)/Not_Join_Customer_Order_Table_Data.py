...
def make_data_file(CASE):
    ...
    fd = open(path + '/data_files/' + str(CASE) + '_Not_Join_Customer_Order_Data' + '.txt', 'w')
    Order_Data_Fd = open(path + '/data_files/' + str(CASE) + '_Order_Data' '.txt', 'r')
    Customer_Data_Fd = open(path + '/data_files/' + str(CASE) + '_Customer_Data' + '.txt', 'r')
